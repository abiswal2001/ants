...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        if self.direction == 1:
            ...
            if self.place.entrance == colony.beehive:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while i < self.min_range:
            ...
            i+=1
        while not place == beehive and i <= self.max_range:
            if place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        length, i = len(self.place.bees), 0
        while i < len(self.place.bees):
            if self.armor <= amount:
                Insect.reduce_armor(self.place.bees[i], amount + self.damage)
            else:
                Insect.reduce_armor(self.place.bees[i], amount)
            if len(self.place.bees) == length:
                i += 1
            else:
...
# END Problem 13
        ...
        if not self.is_true:
            ...
            while current_place:
                if current_place.ant:
                    if not current_place.ant.is_container and not current_place.ant.has_doubled:
                        current_place.ant.damage *= 2
                        current_place.ant.has_doubled = True
                    if current_place.ant.is_container:
                        if not current_place.ant.has_doubled:
                            current_place.ant.damage *= 2
                            current_place.ant.has_doubled = True
                        if current_place.ant.contained_ant and not current_place.ant.contained_ant.has_doubled:
                            current_place.ant.contained_ant.damage *= 2
                            current_place.ant.contained_ant.has_doubled = True
                current_place = current_place.exit
            ScubaThrower.action(self, colony)
            QueenAnt.has_doubled = True
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if amount >= self.armor and self.is_true:
            bees_win()
        else:
            Insect.reduce_armor(self, amount)
        # END Problem 13
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        self.damage = 2 - self.insects_shot * .05
        if self.damage - distance * .2 > 0:
            return self.damage - distance * .2
        else:
            return 0
        # END Problem OPTIONAL
